...
const MemeCanvas = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    console.log(e.target.files, URL.createObjectURL(e?.target?.files?.[0] ?? new Blob()));
    setMemeImage(URL.createObjectURL(e?.target?.files?.[0] ?? new Blob()));
  }

  const URLImage = ({ src, ...rest }: { src: any; x?: number; y?: number }) => {
    const [image] = useImage(memeImage || "https://konvajs.org/assets/yoda.jpg", "anonymous");
  ...
  return (
    <div ref={containerRef} style={{ 
      ...
      <div className="flex flex-col gap-2 w-full" style={{ maxWidth: "600px" }}>
        <input type="file" name="image" id="image" onChange={handleImageUpload} className="w-full" />
        <input type="text" className="meme-text border-2 border-gray-300 rounded-md p-2 w-full" placeholder="Top Text" value={topText || ""} onChange={(e) => setTopText(e.target.value)} />
        <input type="text" className="meme-text border-2 border-gray-300 rounded-md p-2 w-full" placeholder="Bottom Text" value={bottomText || ""} onChange={(e) => setBottomText(e.target.value)} />
      </div>
    </div>
  );
};
